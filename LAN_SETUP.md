...
# Start on all interfaces, port 8000
python3 main.py
...
# Set up port forwarding (device sees localhost:8000)
...
**How it works**: `adb reverse` makes device's port 8000 map to host's port 8000.

**Use URL**: `http://localhost:8000` in the app
...
**Use URL**: `http://localhost:8000` or `http://127.0.0.1:8000`
...
# Device: Browser to http://<host_ip>:8000
...
|-------------|--------------|
| Emulator (adb reverse) | `http://localhost:8000` |
| Emulator (no reverse) | `http://10.0.2.2:8000` |
| Physical (USB) | `http://localhost:8000` |
| Physical (LAN) | `http://192.168.1.x:8000` |
...
curl -I http://localhost:8000/api/v1/runtime/events
...
**Check**:
1. Relay is running: `curl http://localhost:8000/`
...
URL: http://<your_lan_ip>:8000 (physical on LAN)
API Key: <optional>
```
